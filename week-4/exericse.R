...
team.data <- mutate(team.data, TTSRatio = TOV / STL)

# Sort the teams from lowest turnover/steal ratio to highest
sorted_team.data <- arrange(team.data, TTSRatio)

# Get the team that had the highest Total Rebounds (TRB) only with the columns 
# Team and TRB  *using one line of code*
most_rebounds <- select(filter(team.data, TRB == max(TRB)), Team)
...
type_2_groups <- group_by(pokemon, Type.2)
...
# within each group.
type_2_summaries <- summarize(type_2_groups, mean = mean(Speed), maxHP = max(HP))
...
generation_aggregate <- group_by(pokemon, Generation) %>% filter(Legendary == "False") %>% summarize(minAttack = min(Attack), maxAttack = max(Attack), minDefense = min(Defense), maxDefense = max(Defense))
# Can you do it in one line?
View(generation_aggregate)
